...
function FactureDivers({
  ...
  const genererPDFIntégral = (cli, docInstance = null) => {
    ...
    doc.text('PROFACT - FICHE & FACTURE COMPLÈTE', 14, 18);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Pays', cli.pays || 'RDC'],
        ['Type de Facture', cli.typeFacture || cli.type || 'Divers'],
    ...
    if (isSingle) {
      doc.save(`Facture_Complete_${cli.matricule || cli.bail || 'Client'}.pdf`);
    }
  };
  ...
  const telechargerToutEnPDF = () => {
    ...
    doc.save('Toutes_les_Factures_Diverses.pdf');
  };
    ...
    <ConteneurSection as={motion.div} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      {listeFactures.length === 0 ? (
        <MessageVide>
          ...
          <p>Aucune facture diverse trouvée.</p>
        </MessageVide>
      ) : (
        <GrilleFactures>
