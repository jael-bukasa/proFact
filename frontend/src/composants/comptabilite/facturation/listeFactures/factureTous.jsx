...
export default function FactureTous({ listeFactures = [], formaterDateFr }) {
  ...
  return (
    <ConteneurSection as={motion.div} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      {listeFactures.length === 0 ? (
        <MessageVide>Aucune facture enregistrée ne correspond à vos critères.</MessageVide>
      ) : (
        <>
                ...
                {listeFactures.map((cli, index) => {
                  ...
                  return (
                    <LigneTableau key={cli.id || index}>
                      ...
                      <CelluleData>
                        <BadgeType>{cli.typeFacture || cli.type || 'Loyers'}</BadgeType> 
